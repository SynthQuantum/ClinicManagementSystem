...
public class AuthController : ControllerBase
{
    ...
    public async Task<IActionResult> Login([FromBody] LoginRequest request)
    {
        ...
        if (user is null || !user.IsActive)
        {
            ...
            await WriteAuditAsync("Authentication", "LoginFailed", null,
                $"Login attempt failed for email '{request.Email}' (user not found or inactive)");
        ...
        if (result.IsLockedOut)
        {
            ...
            await WriteAuditAsync("Authentication", "AccountLockedOut", user.Id,
                $"Account locked out after too many failed attempts for '{request.Email}'");
        ...
        if (!result.Succeeded)
        {
            ...
            await WriteAuditAsync("Authentication", "LoginFailed", user.Id,
                $"Invalid password for '{request.Email}'");
            return Unauthorized(new { message = "Invalid credentials." });
        }
    ...
    private Task WriteAuditAsync(string entityName, string actionType, Guid? userId, string description)
    {
        return _auditLogService.CreateAsync(new Models.Entities.AuditLog
        {
